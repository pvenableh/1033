    ...
    <template #default>
      ...
      >
        <p class="block mt-2 font-medium text-gray-900 dark:text-gray-200">
          No units loaded. <br />What are you waiting for?
        </p>
        <VButton class="mt-2" @click="fetchUnits()" variant="primary">
          Fetch Units</VButton
        >
      </div>
      <ul class="space-y-6" v-else-if="units.length > 0">
...
async function fetchUnits() {
  loading.value = true
  try {
    const { data } = await $directus.items('units').readByQuery({
      // filter: {
