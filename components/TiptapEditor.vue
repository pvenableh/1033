<template>
  <div class="tiptap-editor-wrapper">
    ...
    >
          ...
          <template #panel="{ close }">
            <div class="p-2 space-y-1 min-w-32">
              <button
                ...
                @click="editor.chain().focus().toggleHeading({ level }).run(); close()"
              >
                <span :class="`text-${level === 1 ? 'lg' : level === 2 ? 'base' : 'sm'} font-bold`">
                  H{{ level }}
                </span>
              ...
              <button
                class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                :class="{ 'bg-primary-100 dark:bg-primary-900': editor.isActive('paragraph') && !editor.isActive('heading') }"
                @click="editor.chain().focus().setParagraph().run(); close()"
              >
          ...
          <template #panel="{ close }">
            <div class="p-2 space-y-1 min-w-40">
              <button
                ...
                class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                @click="editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run(); close()"
              >
              ...
              <template v-if="editor.isActive('table')">
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().addColumnBefore().run(); close()"
                >
                  <UIcon name="i-lucide-columns-3" class="w-4 h-4" />
                  Add Column Before
                </button>
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().addColumnAfter().run(); close()"
                >
                  <UIcon name="i-lucide-columns-3" class="w-4 h-4" />
                  Add Column After
                </button>
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().addRowBefore().run(); close()"
                >
                  <UIcon name="i-lucide-rows-3" class="w-4 h-4" />
                  Add Row Before
                </button>
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().addRowAfter().run(); close()"
                >
                  <UIcon name="i-lucide-rows-3" class="w-4 h-4" />
                  Add Row After
                </button>
                ...
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().deleteColumn().run(); close()"
                >
                ...
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().deleteRow().run(); close()"
                >
                  <UIcon name="i-lucide-trash-2" class="w-4 h-4 text-red-500" />
                  Delete Row
                </button>
                <button
                  class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                  @click="editor.chain().focus().deleteTable().run(); close()"
                >
                  <UIcon name="i-lucide-trash-2" class="w-4 h-4 text-red-500" />
                  Delete Table
                </button>
      ...
      <div v-if="mode === 'full' && allowUploads" class="flex items-center border-r border-gray-300 dark:border-gray-600 pr-1 mr-1">
        <UPopover :popper="{ placement: 'bottom-start' }">
          <ToolbarButton
            icon="i-lucide-image"
            title="Insert Image"
          ...
          <template #panel="{ close }">
            <div class="p-2 space-y-1 min-w-40">
              ...
              <button
                class="w-full text-left px-3 py-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2"
                @click="showImageUrlInput = true; close()"
              >
    ...
    <UModal v-model="showImageUrlInput">
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <UFormGroup label="Alt Text (optional)">
            <UInput
              v-model="imageAlt"
        ...
        <template #footer>
          <div class="flex justify-end gap-2">
            <UButton color="gray" variant="ghost" @click="showImageUrlInput = false">
              Cancel
            </UButton>
            <UButton color="primary" @click="insertImageFromUrl" :disabled="!imageUrl">
              Insert
            </UButton>
          </div>
        </template>
...
const filteredFiles = computed(() => {
  ...
  if (fileSearchQuery.value) {
    ...
    result = result.filter(f =>
      ...
      f.filename_download?.toLowerCase().includes(query)
    );
  }
