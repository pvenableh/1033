...
const handleFiles = async (files) => {
	...
	try {
		...
		// Update each file with the folder ID
		const updatedFiles = await Promise.all(
			uploadedFiles.map(async (file) => {
				try {
					const updated = await updateFile(file.id, {
						folder: folderId,
						// You can add additional metadata from processedFiles if needed
						title: processedFiles.find((pf) => pf.sanitizedName === file.filename_download)?.originalName,
					});
					return updated;
				} catch (updateError) {
					...
					toast.add({
						title: 'Warning',
						description: `Failed to update file metadata for ${file.filename_download}`,
						color: 'yellow',
					});
					return file;
				}
		...
		updatedFiles.forEach((file) => {
			...
			// Check if file type is allowed and handle accordingly
			const processedFile = processedFiles.find((pf) => pf.sanitizedName === file.filename_download);
			...
			if (processedFile?.type.startsWith('image/')) {
				...
				// For non-image files, create a more informative link
