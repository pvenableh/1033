<template>
	<div>
		...
		<div v-else-if="activities.length === 0" class="text-sm text-gray-400 text-center py-4">No recent activity</div>
		...
		<div v-else class="space-y-3 max-h-48 overflow-y-auto">
			<div v-for="activity in activities" :key="activity.id" class="flex items-start gap-3">
				<Avatar :src="getAvatarUrl(activity.user)" :alt="activity.user?.first_name || 'User'" size="xs" />
				...
				<Icon :name="activity.icon" class="w-4 h-4 text-gray-400 flex-shrink-0" />
			</div>
		</div>
	</div>
</template>
