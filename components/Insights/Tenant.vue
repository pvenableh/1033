...
<template>
	<div class="tenants">
		...
		<p v-if="tenant.leases && activeLease" class="tracking-wide uppercase text-sm"><span class="opacity-50">Lease:</span> Expires on {{ getFriendlyDateTwo(activeLease.finish) }} </p>
		<p v-else class="tracking wide text-red uppercase font-bold text-sm">No Lease!</p>
		<a v-if="tenant.leases && activeLease" :href="'https://admin.1033lenox.com/assets/' + activeLease.file" class="mt-2 btn">View Lease Document</a>
