...
const allVehicles = computed(() => {
  return props.units.flatMap(unit => unit.units_id?.vehicles || [])
})
</script>
  ...
  <Card>
    <CardHeader class="pb-3">
      <div class="flex items-center justify-between">
        <div>
          <CardTitle class="text-base">Your Vehicles</CardTitle>
          <CardDescription>Registered vehicles</CardDescription>
        </div>
        <Icon name="heroicons:truck" class="h-5 w-5 text-muted-foreground" />
      </div>
    </CardHeader>
    <CardContent>
      <div v-if="allVehicles.length > 0" class="space-y-3">
        <div
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-muted-foreground">
              ...
              <span v-if="vehicle.color && vehicle.license_plate"> &bull; </span>
      ...
      <div v-else class="py-8 text-center">
        <Icon name="heroicons:truck" class="h-12 w-12 mx-auto mb-3 text-muted-foreground opacity-20" />
        <p class="text-sm text-muted-foreground">No vehicles registered.</p>
        <nuxt-link
          to="/account"
