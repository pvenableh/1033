...
const accessRequestSchema = z
	.object({
		...
		unit_id: z.number({required_error: 'Please select a unit'}).or(z.string().min(1, 'Please select a unit')),
...
const {handleSubmit, isSubmitting, setFieldError, values} = useForm<AccessRequestFormValues>({
...
const onSubmit = handleSubmit(async (formValues) => {
	...
	try {
		...
		const unitId = typeof formValues.unit_id === 'string' ? parseInt(formValues.unit_id) : formValues.unit_id;
...
<template>
	<div
		...
		<Alert v-if="submitSuccess" class="my-4" variant="default">
			...
			<p class="text-sm">Your access request has been submitted. You will receive an email once approved.</p>
		</Alert>
		...
		<form v-if="!submitSuccess" @submit="onSubmit" class="grid gap-4">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<VeeField v-slot="{field, errors}" name="unit_id">
					<div class="space-y-2">
						<Label for="unit_id" class="text-sm font-medium leading-none">
							Unit
							<span class="text-xs text-muted-foreground ml-1">(Your unit at 1033 Lenox)</span>
						</Label>
						<Select
							id="unit_id"
				...
				<VeeField v-slot="{field, errors}" name="residency_type">
					<div class="space-y-2">
						<Label for="residency_type" class="text-sm font-medium leading-none">Residency Type</Label>
						<Select
							id="residency_type"
							:model-value="field.value"
			...
			<VeeField v-slot="{field, errors}" name="agreement">
				<div class="space-y-2">
					<div class="flex items-center space-x-2">
						<Checkbox id="agreement" :checked="field.value" @update:checked="field.onChange" />
