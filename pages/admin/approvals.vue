...
const hasAnyPermission = computed(() => {
	return (
		pendingItems.value.permissions.pets ||
...
const totalPending = computed(() => {
	return pendingItems.value.pets.length + pendingItems.value.vehicles.length + pendingItems.value.leases.length;
});
		...
		<div class="container mx-auto px-6 py-8">
			...
			<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
				<div>
					...
					<p class="text-gray-600 dark:text-gray-400 mt-1">Review and approve member submissions</p>
				</div>
			...
			<div v-else-if="!hasAnyPermission" class="text-center py-12">
				...
				<p class="text-gray-600 dark:text-gray-400">You do not have permission to approve any items.</p>
			</div>
			...
			<template v-else>
				...
				<div v-if="totalPending === 0" class="text-center py-12">
					...
					<p class="text-gray-600 dark:text-gray-400">There are no items pending approval.</p>
				</div>
								...
								<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
									<div class="flex items-center gap-4">
										<div class="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-lg">
											<Icon name="i-heroicons-heart" class="w-6 h-6 text-amber-600 dark:text-amber-400" />
										</div>
										<div>
								...
								<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
									<div class="flex items-center gap-4">
										<div class="p-3 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
											<Icon name="i-heroicons-truck" class="w-6 h-6 text-blue-600 dark:text-blue-400" />
										</div>
										<div>
								...
								<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
									<div class="flex items-center gap-4">
										<div class="p-3 bg-purple-100 dark:bg-purple-900/30 rounded-lg">
											<Icon name="i-heroicons-document-check" class="w-6 h-6 text-purple-600 dark:text-purple-400" />
										</div>
										<div>
											<h3 class="font-semibold">{{ lease.person?.first_name }} {{ lease.person?.last_name }}</h3>
											<p class="text-sm text-gray-600 dark:text-gray-400">
												{{ formatDate(lease.start) }} - {{ formatDate(lease.finish) }}
											</p>
											<p class="text-xs text-gray-500 mt-1">Submitted {{ formatDate(lease.date_created) }}</p>
										</div>
									</div>
									<div class="flex items-center gap-2">
										<Button
