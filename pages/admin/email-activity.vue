...
async function refreshData() {
	await Promise.all([fetchActivities(true), fetchAnnouncements(), fetchTotalCounts()]);
}
		...
		<div class="container mx-auto px-6 py-8">
			...
			<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
				<div>
					...
					<p class="text-gray-600 dark:text-gray-400 mt-1">Track email opens, clicks, bounces, and delivery status</p>
				</div>
			...
			<div v-if="!hasAccess" class="text-center py-12">
				...
				<p class="text-gray-600 dark:text-gray-400">You don't have permission to view email activity.</p>
			</div>
			...
			<template v-else>
				...
				<Card>
					<div ref="scrollContainer" class="max-h-[60vh] overflow-y-auto" @scroll="handleScroll">
							...
							<template #event-data="{row}">
								<div class="flex items-center gap-2">
									<Icon
										...
										:class="`text-${getEventColor(row.event)}-500`"
										class="w-5 h-5" />
						...
						<div v-if="loadingMore" class="flex items-center justify-center py-4">
							<Icon name="i-heroicons-arrow-path" class="w-5 h-5 animate-spin text-gray-400 mr-2" />
				...
				<div class="mt-4 flex items-center justify-between text-sm text-gray-500">
					<div>
						Showing {{ filteredActivities.length.toLocaleString() }} of {{ activities.length.toLocaleString() }} loaded
						events
					</div>
				...
				<p v-if="totalCounts.total > activities.length" class="mt-2 text-xs text-gray-400 text-center">
					...
					{{ totalCounts.total.toLocaleString() }} total events. Scroll down or click "Load More" to see older events.
				</p>
			</template>
		</div>
	</div>
