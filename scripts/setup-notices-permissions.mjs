...
const PERMISSIONS_CONFIG = {
		...
		member: [
			{
				...
				permissions: {
					_and: [
						...
						{
							_or: [{visibility: {_contains: 'residents'}}, {visibility: {_contains: 'public'}}],
						},
						{
							_or: [{published_at: {_lte: '$NOW'}}, {published_at: {_null: true}}],
						},
						{
	...
	email_activity: {
		...
		board: [{action: 'read', fields: '*', permissions: {}}],
	...
	announcements_people: {
		...
		member: [{action: 'read', fields: '*', permissions: {}}],
		public: [],
	},
};
...
async function createOrUpdatePermission(client, existingPermissions, permData) {
	...
	const existing = existingPermissions.find(
		(p) => p.collection === collection && p.action === action && (policy ? p.policy === policy : p.role === role)
	);
